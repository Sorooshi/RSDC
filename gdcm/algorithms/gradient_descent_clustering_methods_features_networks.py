...
class GDCMfn:
    ...
    @staticmethod
    def softmax_fn(a_vector):
        """ apply softmax function on the given vector"""
        return jnp.exp(a_vector) / sum(jnp.exp(a_vector))

    @staticmethod
    def sigmoid_fn(a_vector):
        """ apply sigmoid function on the given vector"""
        return jnp.divide(1, (1 + jnp.exp(a_vector)))
